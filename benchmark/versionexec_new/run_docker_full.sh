...
BENCHMARK_FILE="${SCRIPT_DIR}/versionexec_new.jsonl"
...
DOCKER_IMAGE="${DOCKER_IMAGE:-bigcodebench/bigcodebench-evaluate:latest}"
  
echo "==============================================="  
echo "Mode: RUN existing official image from Docker Hub"  
...
echo "Pulling official image with platform linux/amd64..."  
...
docker run --rm --platform=linux/amd64 \
    ...
    -v "${SCRIPT_DIR}:/app" \
    -e BIGCODEBENCH_OVERRIDE_PATH=/app/data/versionexec_new.jsonl \
    "${DOCKER_IMAGE}" \
    --split complete \
    --subset full \
