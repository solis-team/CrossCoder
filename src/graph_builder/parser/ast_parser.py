...
class CodeComponent:
    ...
    # Unresolved dependencies that couldn't be matched during initial parsing
...
def categorize_components_by_type(component_ids: List[str], all_components: Dict[str, 'CodeComponent']) -> Dict[str, List[str]]:
    ...
    """
    categorized = {"class": set(), "function": set(), "variable": set()}
    ...
    for comp_id in component_ids:
        if comp_id in all_components:
            ...
            if comp_type in categorized:
                categorized[comp_type].add(comp_id)
...
class ImportCollector(ast.NodeVisitor):
    ...
    def __init__(self, current_file_path: str = "", repo_modules: Set[str] = None, third_party_modules: Set[str] = None):
        self.current_file_path = current_file_path
        self.repo_modules = repo_modules or set()
        self.third_party_modules = third_party_modules or set()
        self.imports = set()
        self.from_imports = {}  
        self.import_aliases = {}  
    ...
    def _resolve_module_path(self, module_name: str, level: int = 0) -> str:
        ...
        if module_name:
            ...
            if level == 0 and self.current_file_path:
                ...
                # Check for third-party modules at root level for imports without dots
                if "." not in module_name:
                    for third_party_path in self.third_party_modules:
                        if third_party_path.startswith(f"{module_name}/") or third_party_path == f"{module_name}.py":
                            return module_path
                
                # Check if module is a directory/package containing Python files
                module_dir = module_name.replace(".", "/")
                
                # First check if it has an __init__.py (it's a package)
                init_file = f"{module_dir}/__init__.py"
                if init_file in self.repo_modules:
                    return init_file
                
                # For wrapped repositories, also check subdirectories
                for repo_module in self.repo_modules:
                    if repo_module.endswith(f"/{init_file}"):
                        return repo_module
                    if repo_module.endswith(f"/{module_path}"):
                        return repo_module
                
                # Then check if there are any files under this directory
                found_files = []
                for repo_module in self.repo_modules:
                    if repo_module.startswith(f"{module_dir}/") and repo_module.endswith(".py"):
                        found_files.append(repo_module)
                    # Also check wrapped structure
                    elif f"/{module_dir}/" in repo_module and repo_module.endswith(".py"):
                        found_files.append(repo_module)
                        
                if found_files:
                    # Return the __init__.py if it exists, otherwise return the directory
                    for repo_module in self.repo_modules:
                        if repo_module.endswith(f"/{module_dir}/__init__.py"):
                            return repo_module
                    return found_files[0].split("/")[:-1] and "/".join(found_files[0].split("/")[:-1]) or module_dir
...
class DependencyCollector(ast.NodeVisitor):
    ...
    def visit_Name(self, node: ast.Name):
        ...
        if isinstance(node.ctx, ast.Load):
            if hasattr(node, 'parent') and isinstance(node.parent, ast.Attribute) and node.parent.value == node:
                return
    ...
    def _process_attribute(self, node: ast.Attribute, is_method_call=False):
        ...
        if isinstance(current, ast.Name):
            ...
            # If no resolution found, add as unresolved dependency
            # This handles cases like rsa.newkeys where rsa is imported but newkeys is not directly found
            if len(parts) > 1:
                self.dependencies.add(parts[-1])
            else:
                # For simple attribute access, add as unresolved
    ...
    def _add_dependency(self, name):
        ...
        if name in self.imports:
            module_path = name
            if module_path not in self.repo_modules:
                return
...
class DependencyParser:
    ...
    def __init__(self, repo_path: str):
        ...
        self.third_party_modules: Set[str] = set()
        self.external_knowledge: Dict[str, Dict[str, List[str]]] = {}
        self.unresolved_dependencies: Dict[str, Set[str]] = {}
    ...
    def parse_repository(self):
        ...
        self._discover_third_party_modules()
        ...
        self._resolve_cross_module_dependencies()

        self._add_noise_to_components()

        self._update_out_counts()

        self._filter_external_entities()
    ...
    def _parse_file(self, file_path: str, relative_path: str, module_path: str):
        ...
        try:
            ...
            import_collector = ImportCollector(relative_path, self.modules, self.third_party_modules)
    ...
    def _resolve_dependencies(self):
        ...
        for component_id, component in self.components.items():
            ...
            try:
                ...
                tree = ast.parse(source)
                add_parent_to_nodes(tree)
                import_collector = ImportCollector(component.relative_path, self.modules, self.third_party_modules)
                import_collector.visit(tree)
                component_node = None
                ...
                if component_node:
                            ...
                            if dep in self.components:
                                ...
                                if dep_module in self.modules:
                                    for comp_id in self.components:
                                        ...
                                        unresolved_deps.add(dep_name)
                                else:
                                    unresolved_deps.add(dep_name)
                        else:
                            unresolved_deps.add(dep)
                    
                    self.unresolved_dependencies[component_id] = unresolved_deps                    
                    ...
                    component.unresolved_dependencies = list(unresolved_deps)
    ...
    def _find_entity_source(self, package_init_path: str, entity_name: str) -> Optional[str]:
        ...
        """
        # First check if entity is defined in __init__.py itself
        init_entity_id = f"{entity_name}@{package_init_path}"
        if init_entity_id in self.components:
    ...
    def _get_third_party_entities(self, package_name: str, import_type: str = 'package') -> List[str]:
        """
        Get all non-underscore entities from a third-party package.
        
        Args:
            package_name: Name of the third-party package
            import_type: 'package' for import pkg, 'direct' for from pkg import x
            
        Returns:
            List of entity IDs from the third-party package
        """
        entities = []
        package_prefix = f"{package_name}/"
        
        package_init_path = f"{package_name}/__init__.py"
        init_in_subdir = None
        for module_path in self.modules:
            if module_path.endswith(f"/{package_init_path}"):
                init_in_subdir = module_path
                break
        
        if init_in_subdir:
            for comp_id in self.components:
                if comp_id.endswith(f"@{init_in_subdir}"):
                    entity_name = comp_id.split("@")[0]
                    if not entity_name.startswith("_"):
                        comp_type = self.components[comp_id].component_type
                        if comp_type in ["class", "function", "variable"]:
                            entities.append(comp_id)
        else:
            if package_init_path in self.modules:
                for comp_id in self.components:
                    if comp_id.endswith(f"@{package_init_path}"):
                        entity_name = comp_id.split("@")[0]
                        if not entity_name.startswith("_"):
                            comp_type = self.components[comp_id].component_type
                            if comp_type in ["class", "function", "variable"]:
                ...
                for comp_id in self.components:
                    module_path = comp_id.split("@")[1]
                    if module_path.startswith(package_prefix) or module_path == f"{package_name}.py":
                        entity_name = comp_id.split("@")[0]
                        if not entity_name.startswith("_"):
                            comp_type = self.components[comp_id].component_type
                            if comp_type in ["class", "function", "variable"]:
            ...
            for file in files:
                ...
                try:
                        ...
                        if module_path in self.modules:
                                    ...
                                    if name != '*':  
                                        ...
                                        if entity_id in self.components:
                                            ...
                                            if module_path.endswith('__init__.py'):
                                                package_dir = module_path.replace('/__init__.py', '')
                                                for comp_id in self.components:
                                                    if comp_id.startswith(f"{original_name}@{package_dir}/"):
                                                        external_entities.add(comp_id)
                                                        break
                        else:
                            ...
                            if package_init_path in self.modules:
                                for name in imported_names:
                                    if name != '*':
                                        package_dir = package_init_path.replace("/__init__.py", "")
                                        potential_module_path = f"{package_dir}/{name}.py"
                                        
                                        if potential_module_path in self.modules:
                                            for comp_id in self.components:
                                                if comp_id.endswith(f"@{potential_module_path}"):
                                                    comp_type = self.components[comp_id].component_type
                                                    if comp_type in ["class", "function", "variable"]:
                                                        external_entities.add(comp_id)
                                        else:
                                            ...
                                            if actual_source:
                                                original_name = import_collector.original_names.get(name, name)
                                                external_entities.add(f"{original_name}@{actual_source}")
                            else:
                                package_dir = module_path.replace(".py", "")
                                for name in imported_names:
                                    if name != '*':
                                        potential_file_path = f"{package_dir}/{name}.py"
                                        
                                        if potential_file_path in self.modules:
                                            for comp_id in self.components:
                                                if comp_id.endswith(f"@{potential_file_path}"):
                                                    comp_type = self.components[comp_id].component_type
                                                    if comp_type in ["class", "function", "variable"]:
                                                        external_entities.add(comp_id)
                                        else:
                                            package_name = module_path.split("/")[0].replace(".py", "")
                                            is_third_party = False
                                            for tp_module in self.third_party_modules:
                                                if tp_module.startswith(f"{package_name}/") or tp_module == f"{package_name}.py":
                                                    is_third_party = True
                                                    break
                                            ...
                                            if is_third_party:
                                                for name in imported_names:
                                                    if name != '*':
                                                        ...
                                                        package_init_path_tp = f"{package_name}/__init__.py"
                                                        init_in_subdir = None
                                                        for tp_module in self.third_party_modules:
                                                            if tp_module.endswith(f"/{package_init_path_tp}"):
                                                                init_in_subdir = tp_module
                                                                break
                                                        
                                                        if init_in_subdir:
                                                            actual_package_dir = init_in_subdir.replace("/__init__.py", "")
                                                            potential_file = f"{actual_package_dir}/{original_name}.py"
                                                            
                                                            if potential_file in self.modules:
                                                                for comp_id in self.components:
                                                                    if comp_id.endswith(f"@{potential_file}"):
                                                                        comp_type = self.components[comp_id].component_type
                                                                        if comp_type in ["class", "function", "variable"]:
                                                                            external_entities.add(comp_id)
                                                            else:
                                                                entity_id = f"{original_name}@{init_in_subdir}"
                                                                if entity_id in self.components:
                                                                    external_entities.add(entity_id)
                                                        else:
                                                            entity_id = f"{original_name}@{module_path}"
                                                            if any(comp_id == entity_id for comp_id in self.components):
                                                                external_entities.add(entity_id)
                                                            else:
                                                                for comp_id in self.components:
                                                                    if comp_id.startswith(f"{original_name}@{package_name}/"):
                                                                        external_entities.add(comp_id)
                                                                        break
                                            else:
                                                for name in imported_names:
                                                    if name != '*':
                                                        original_name = import_collector.original_names.get(name, name)
                                                        if module_path.endswith('.py') and not module_path.count('/'):
                                                            package_name = module_path.replace('.py', '')
                                                            package_init = f"{package_name}/__init__.py"
                                                            external_entities.add(f"{original_name}@{package_init}")
                                                        else:
                                                            external_entities.add(f"{original_name}@{module_path}")
                    ...
                    for module_path in import_collector.imports:
                        ...
                        if is_third_party:
                            third_party_entities = self._get_third_party_entities(package_name)
                            external_entities.update(third_party_entities)
                        elif package_name not in STANDARD_MODULES and module_path not in self.modules:
                            for comp_id in self.components:
                                if comp_id.endswith(f"@{module_path}"):
                                    comp_type = self.components[comp_id].component_type
                                    if comp_type in ["class", "function", "variable"]:
                    ...
                    self.external_knowledge[relative_path] = {
                        ...
                        'external_entity': list(external_entities)
                    }
    ...
    def _resolve_cross_module_dependencies(self):
        """
        Cross-reference unresolved dependencies with external entities to resolve cross-module dependencies.
        """
        for component_id, component in self.components.items():
            if component_id not in self.unresolved_dependencies:
                continue
                
            relative_path = component.relative_path
            if relative_path not in self.external_knowledge:
                continue
                
            unresolved_names = self.unresolved_dependencies[component_id]
            ...
            for external_entity in external_entities:
                entity_name = external_entity.split("@")[0]
                if entity_name in unresolved_names:
                    resolved_entities.append(external_entity)
            
            if resolved_entities:
                categorized_resolved = categorize_components_by_type(resolved_entities, self.components)
                
                for comp_type, entities in categorized_resolved.items():
                    existing_entities = set(component.outgoing_calls[comp_type])
                    new_entities = set(entities)
                    component.outgoing_calls[comp_type] = list(existing_entities | new_entities)
                
                component.out_count = sum(len(v) for v in component.outgoing_calls.values())
                
                resolved_names = {entity.split("@")[0] for entity in resolved_entities}
                component.unresolved_dependencies = [dep for dep in component.unresolved_dependencies if dep not in resolved_names]
    ...
    def _filter_external_entities(self):
        """
        Filter external entities - only keep entities that exist in components.
        """
        for file_path, knowledge in self.external_knowledge.items():
            filtered_entities = []
            for entity_id in knowledge['external_entity']:
                # Only keep entities that exist in components
                if entity_id in self.components:
                    filtered_entities.append(entity_id)
            
            knowledge['external_entity'] = filtered_entities
            
            # Cross-reference with unresolved dependencies
            for component_id, component in self.components.items():
                if component.relative_path == file_path and component_id in self.unresolved_dependencies:
                    unresolved_names = self.unresolved_dependencies[component_id]
                    
                    for entity_id in filtered_entities:
                        entity_name = entity_id.split("@")[0] if "@" in entity_id else entity_id
                        if entity_name in unresolved_names:
                            # For entities in components, add to appropriate type
                            if entity_id in self.components:
                                comp_type = self.components[entity_id].component_type
                                if comp_type in component.outgoing_calls:
                                    if entity_id not in component.outgoing_calls[comp_type]:
                                        component.outgoing_calls[comp_type].append(entity_id)
                            else:
                                # For external entities (like external libraries), add to class type
                                # since most external dependencies are classes or modules
                                if entity_id not in component.outgoing_calls['class']:
                                    component.outgoing_calls['class'].append(entity_id)
    ...
    def _discover_third_party_modules(self):
        """
        Discover third-party packages in the repository.
        For wrapped repositories, only consider packages outside the main project as third-party.
        """
        repo_items = os.listdir(self.repo_path)
        
        # Detect if this is a wrapped repository (has single main directory)
        python_dirs = [item for item in repo_items if os.path.isdir(os.path.join(self.repo_path, item)) and not item.startswith('.')]
        
        if len(python_dirs) == 1:
            # Likely a wrapped repository, check if main dir contains Python packages
            main_dir = python_dirs[0]
            main_path = os.path.join(self.repo_path, main_dir)
            main_subdirs = [item for item in os.listdir(main_path) if os.path.isdir(os.path.join(main_path, item)) and not item.startswith('.')]
            
            # If main directory contains multiple Python packages, treat them as internal modules
            has_python_packages = any(
                os.path.exists(os.path.join(main_path, subdir, '__init__.py')) or
                any(f.endswith('.py') for f in os.listdir(os.path.join(main_path, subdir)) if os.path.isfile(os.path.join(main_path, subdir, f)))
                for subdir in main_subdirs if os.path.isdir(os.path.join(main_path, subdir))
            )
            
            if has_python_packages:
                # This is a wrapped repository, don't treat main project as third-party
                # Only look for actual third-party packages at root level or inside main dir
                for item in repo_items:
                    if item != main_dir:
                        item_path = os.path.join(self.repo_path, item)
                        if os.path.isdir(item_path) and not item.startswith('.'):
                            self._add_third_party_modules_from_dir(item_path, self.repo_path)
                return
        
        # Regular repository structure - treat all root directories as potential third-party
        for item in repo_items:
            item_path = os.path.join(self.repo_path, item)
            if os.path.isdir(item_path) and not item.startswith('.'):
                self._add_third_party_modules_from_dir(item_path, self.repo_path)
    
    def _add_third_party_modules_from_dir(self, dir_path, repo_root):
        """
        Add Python modules from a directory as third-party modules.
        """
        ...
        for root, _, files in os.walk(dir_path):
            for file in files:
                if file.endswith('.py'):
                    ...
                    module_path = self._file_to_module_path(relative_path)
                    self.third_party_modules.add(module_path)
                    if has_python_files:
                        break
